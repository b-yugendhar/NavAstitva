    ...
    return (
      <div className="max-w-2xl mx-auto my-12 bg-white border border-slate-200 rounded-3xl p-8 text-center space-y-6 shadow-xs">
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-black text-slate-900">Worker Dashboard</h2>
          <p className="text-sm text-slate-600 max-w-md mx-auto">
            You don't have an active worker profile yet. Take a quick skill assessment or load sample trade data to explore.
          </p>
        </div>
        <div className="flex flex-wrap items-center justify-center gap-4 pt-2">
          <button
            onClick={() => setIsAiModalOpen(true)}
            className="px-6 py-3 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white rounded-xl text-sm font-bold transition-all shadow-xs flex items-center gap-2 cursor-pointer"
          >
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            onClick={() => setIsAiModalOpen(true)}
            className="px-4 py-2.5 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white rounded-xl text-xs font-bold transition-all shadow-xs flex items-center gap-1.5 cursor-pointer"
          >
            <Sparkles className="w-4 h-4" />
            <span>Skill Scoring</span>
          </button>
          <button
            onClick={() => onNavigate('evidence')}
            className="px-4 py-2.5 bg-orange-100 hover:bg-orange-200 text-orange-800 rounded-xl text-xs font-bold transition-all shadow-xs flex items-center gap-1.5 cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Trust Score Card */}
        <div className="bg-gradient-to-br from-orange-600 to-red-700 rounded-2xl text-white p-6 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <div className="flex items-baseline gap-2 mb-2">
              ...
              <span className="text-orange-200 font-semibold text-lg">/ 100</span>
            </div>

            <p className="text-xs text-white/90 leading-relaxed mb-4">
              Based on {worker.skills.filter(s => s.isVerified).length} verified skills, {worker.jobsCompleted} completed jobs, and {worker.ratingsAverage}★ ratings.
            </p>

            <div className="space-y-2 text-xs border-t border-orange-500/60 pt-3">
              <div className="flex justify-between">
                <span className="text-white/80">Verified Skills</span>
                <span className="font-bold">30 / 30</span>
              </div>
              <div className="flex justify-between">
                <span className="text-white/80">Authenticated Evidence</span>
                <span className="font-bold">20 / 25</span>
              </div>
              <div className="flex justify-between">
                <span className="text-white/80">Completed Work History</span>
                <span className="font-bold">18 / 25</span>
              </div>
              <div className="flex justify-between">
                <span className="text-white/80">Employer Ratings (4.8★)</span>
          ...
          <button
            ...
            className="w-full mt-5 py-2.5 rounded-xl bg-white text-orange-900 hover:bg-orange-50 text-xs font-bold tracking-wide transition-all text-center cursor-pointer shadow-xs"
          >
            View Breakdown
          </button>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs flex flex-col justify-between">
            ...
            {activeAgreement ? (
              <div className="space-y-3">
                ...
                <div className="text-xs text-slate-600 space-y-1">
                  ...
                  <div><strong>Escrow Status:</strong> <span className="text-orange-700 font-bold">₹12,000 DEMO-HELD</span></div>
                </div>

                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-600">
                  <div className="font-semibold text-slate-900 mb-1">Terms:</div>
          ...
          <div className="flex gap-2 mt-4">
            <button
              ...
              className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white text-xs font-bold transition-all text-center cursor-pointer"
            >
              Open Agreement
            </button>
            <button
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs flex flex-col justify-between">
          ...
          <button
            onClick={() => setIsAiModalOpen(true)}
            className="w-full mt-4 py-2.5 rounded-xl border border-orange-300 hover:bg-orange-50 text-orange-700 text-xs font-bold transition-all text-center cursor-pointer flex items-center justify-center gap-1.5"
          >
            <Sparkles className="w-3.5 h-3.5 text-orange-500" />
            <span>Score New Skill</span>
          </button>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Matched to your verified skills and ₹{worker.preferredDailyWage} daily rate.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {jobs.map((job) => (
            <div key={job.id} className="p-4 rounded-xl border border-slate-200 bg-slate-50/50 hover:bg-white hover:shadow-md transition-all flex flex-col justify-between">
              ...
              <button
                onClick={() => onNavigate('jobs')}
                className="w-full py-2 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white rounded-lg text-xs font-bold transition-colors cursor-pointer"
              >
                Apply
              </button>
            </div>
